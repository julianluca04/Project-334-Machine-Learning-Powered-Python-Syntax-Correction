...
train_df[cols].to_csv("train_seq2seq_data.csv", index=False)
test_df[cols].to_csv("test_seq2seq_data.csv", index=False)
np.save("train_indices.npy", train_df.index.values)
np.save("test_indices.npy", test_df.index.values)
...
tokenizer_path = "char_tokenizer_final.pkl"
...
def plot_full_error_distribution(distances):
    ...
    plt.savefig("full_error_distribution.png")
    plt.close('all')
