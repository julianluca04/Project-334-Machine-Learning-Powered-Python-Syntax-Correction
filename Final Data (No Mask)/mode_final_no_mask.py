...
####################################################################
######## PHASE 11: TESTING (AI Generated)
####################################################################

def fix_code(buggy_str, model, tokenizer, max_len):
    # 1. Encode the buggy input
    enc_input = tokenizer.encode(buggy_str)
    # Pad to max_len
    enc_input_padded = pad_entry(enc_input, max_len, tokenizer.stoi["<pad>"])
    enc_input_tensor = tf.constant([enc_input_padded])

    # 2. Initialize the decoder input with just <bos>
    dec_input = [tokenizer.stoi["<bos>"]]
    
    # 3. Generate tokens one by one
    for _ in range(max_len - 1):
        # Pad current decoder sequence to max_len for the model
        curr_dec_padded = pad_entry(dec_input, max_len, tokenizer.stoi["<pad>"])
        dec_input_tensor = tf.constant([curr_dec_padded])

        # Predict next token
        predictions = model([enc_input_tensor, dec_input_tensor], training=False)
        
        # Get the prediction for the LAST non-pad token we just provided
        # shape: (1, max_len, vocab_size) -> we want the token at index len(dec_input)-1
        next_token_logits = predictions[0, len(dec_input) - 1, :]
        next_token_id = tf.argmax(next_token_logits).numpy()

        # Stop if model predicts <eos>
        if next_token_id == tokenizer.stoi["<eos>"]:
            break
...
# --- Test on 10 samples from the test set ---
print("\n" + "="*50)
print("SAMPLING MODEL PERFORMANCE (10 EXAMPLES)")
print("="*50)

samples = test_df.sample(10)
for i, (idx, row) in enumerate(samples.iterrows()):
    buggy = row['buggy_clean']
    ground_truth = row['fixed_clean']
    
    predicted_fix = fix_code(buggy, model, tokenizer, max_entry_length)
    
    print(f"\nExample {i+1}:")
    print(f"BUGGY:     {repr(buggy)}")
    print(f"PREDICTED: {repr(predicted_fix)}")
    print(f"ACTUAL:    {repr(ground_truth)}")
    print("-" * 30)